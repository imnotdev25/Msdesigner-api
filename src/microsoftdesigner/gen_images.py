...
def create_img(user_id: str, auth_token: str, prompt: str, *, save_path: str = 'images',
               resolution: str = '1024x1024', boost_count: int = 1, seed: int = None) -> list:
    """
    ...
    def create_form_data(boundary: str, prompt: str, resolution: str, boost_count: int, 
                        seed: int, *, poll_url: str = None) -> str:
        ...
        data_parts = [
            ...
            ('dalle-batch-size', '4'),
            ('dalle-last-prompt', ''),
            ('dalle-last-user-prompt', ''),
            ('dalle-image-response-format', 'UrlWithBase64Thumbnail'),
            ('dalle-seed', str(seed)),
            ('dalle-boost-count', str(boost_count)),
            ('ClientFlights', 'EnableBICForDALLEFlight'),
            ('dalle-hear-back-in-ms', '1000'),
            ('dalle-include-b64-thumbnails', 'true'),
            ('dalle-aspect-ratio-scaling-factor-b64-thumbnails', '0.3'),
            ('dalle-image-size', resolution),
        ]
    ...
    try:
        with httpx.Client() as client:
            ...
            while attempts < max_attempts:
                ...
                if ('image_urls_thumbnail' in current_response and 
                    current_response['image_urls_thumbnail'] and 
                    len(current_response['image_urls_thumbnail']) > 0):
                    
                    image_urls = [img['ImageUrl'] for img in current_response['image_urls_thumbnail'] 
                                if img.get('ImageUrl')]
                ...
                if ('polling_response' in current_response and 
                    'polling_meta_data' in current_response['polling_response']):
